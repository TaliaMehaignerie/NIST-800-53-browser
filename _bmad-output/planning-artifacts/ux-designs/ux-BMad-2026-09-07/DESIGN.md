...
- **ISO Crosswalk entry** — sits inside the Control Detail card, below the statement. `ink-secondary` for the "ISO/IEC 27001:2022" label prefix, then each mapped clause as a small `mono`-type pill (matches the ID-scannability convention) — code only, no clause title: NIST's own source mapping publishes clause numbers without ISO's clause title text, and ISO/IEC 27001 is a licensed standard, so this product doesn't reproduce title text it was never given. A clause NIST flagged as a partial-intent match (its own trailing `*`) keeps that `*` as part of the pill's `mono` text (no new color — `baseline`/`accent` stay reserved per the Colors section) plus one small `ink-secondary` legend line beneath the clause list: "* does not fully satisfy the intent of this control (per NIST's mapping notes)." When none is published, the empty-state text (see EXPERIENCE.md → State Patterns) renders in its place, same position.
