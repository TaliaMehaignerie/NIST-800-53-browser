...
- source_spec: `_bmad-output/implementation-artifacts/spec-search.md`
  summary: The inert Baseline filter on `/search` (45% opacity) remains fully clickable/keyboard-focusable — a sighted mouse user reads "does nothing here" from the dimming, but a keyboard/screen-reader user gets no such signal and can still tab to and activate pills there.
  evidence: Real (blind-hunter), but investigated and NOT a bug to fix by disabling the control: clicking a pill on `/search` still has a real effect via `BaselineFilter`'s existing `updateLinks()`, which rewrites any `main a[href*="/controls/"], main a[href*="/families/"]` link — including the search-result links `SearchBox.astro` renders into the DOM after each query. So a Baseline filter clicked on `/search` legitimately carries `?baseline=` onto whichever result the user clicks through to next, matching this project's established cross-page-persistence pattern (CAP-2). Adding `aria-disabled`/`tabindex="-1"` would remove that real capability, not just a cosmetic mismatch. DESIGN.md's literal spec ("visibly there, visibly not doing anything") is about the page's own rows, not the control's own interactivity. Worth a live-region announcement if this proves genuinely confusing to keyboard users in practice, but not worth disabling.
